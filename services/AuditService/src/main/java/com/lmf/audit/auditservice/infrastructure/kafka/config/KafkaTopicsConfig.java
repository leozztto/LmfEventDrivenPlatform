...
public class KafkaTopicsConfig {
    ...
    @Bean
    public NewTopic fraudApprovedDltTopic() {
        return TopicBuilder.name(KafkaTopics.FRAUD_APPROVED + ".dlt").partitions(3).replicas(1).build();
    }

    @Bean
    public NewTopic fraudRejectedDltTopic() {
        return TopicBuilder.name(KafkaTopics.FRAUD_REJECTED + ".dlt").partitions(3).replicas(1).build();
    }

    @Bean
    public NewTopic inventoryReservedDltTopic() {
        return TopicBuilder.name(KafkaTopics.INVENTORY_RESERVED + ".dlt").partitions(3).replicas(1).build();
    }
